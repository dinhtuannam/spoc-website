...
function Section1() {
    ...
    useEffect(() => {
        const fetchApi = async () => {
            setLoading(true);
            const res = await LayoutService.trangChu.getSection(2);
            if (res) {
    ...
    const onSave = async () => {
        ...
        const result = await callApi(
            ApiRoute.Layout.update(PageEnum.TrangChu, ComponentEnum.DefaultSection, 2),
            {
                method: 'PUT',
                body: JSON.stringify(payload),
            },
            'Lưu section trang chủ thành công',
        );
